...
class IndexHandler(tornado.web.RequestHandler):
    def get_privatekey(self):
        try:
            # data = self.request.files.get('privatekey')[0]['rbody']
            file = self.get_argument('privatekey')
    ...
    def get(self):
        ...
        dir = '/'.join(base_dir.split('/')[:-2])
        key = '/'.join((dir, 'upload/key', user_key, user_key))
        self.render('index.html', ip=ip, port=port, user=user, key=key)
...
def main():
    ...
    parse_command_line()
    options.parse_config_file('../../conf/conf.py')
